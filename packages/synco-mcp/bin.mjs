...
const bundled = join(HERE, "integrations");
const fromRepo = join(HERE, "../../integrations");
const SOURCE = existsSync(join(bundled, "synco-guard.mjs"))
  ? bundled
  : existsSync(join(fromRepo, "synco-guard.mjs"))
    ? fromRepo
    : undefined;
if (!SOURCE) {
  throw new Error("synco-mcp: integrations not found. Reinstall the package.");
}
const HOME = homedir();

const args = process.argv.slice(2).filter((arg, index, list) => !(index === 0 && arg === "init"));
...
if (flag("help")) {
  console.log(`synco-mcp — install the edit guard for every detected agent host

Usage: npx synco-mcp init [hosts...] [options]
...
console.log(DRY ? "synco-mcp — dry run\n" : "synco-mcp\n");
